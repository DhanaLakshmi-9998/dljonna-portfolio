import React,{useContext} from 'react'
import {  FaLinkedinIn } from "react-icons/fa";
import ThemeContext from '../../context';
import { AiFillGithub } from 'react-icons/ai';

const Footer = () => {
  const theme=useContext(ThemeContext);
  const darkMode=theme.state.darkMode;
  return (
    <div id="works" className=" mx-auto m-auto h-[300px]  mt-16 sm:h-[250px] ">
      <div className=" bg-[#000] h-full flex flex-col gap-8 items-center justify-between p-10  sm:p-7"
      style={{border:darkMode?"5px solid #FCA61F":"", borderRadius:"5px"}}
      >
        <h2
          data-aos="zoom-out"
          className=" font-bold text-5xl max-[660px]:text-2xl text-[#FCA61F]"
        >
          Let's Talk
        </h2>
        <div className=" flex items-center justify-center gap-8 sm:gap-5">
          <a
            data-aos="fade-up"
            data-aos-duration="1400"
            href="https://www.linkedin.com/in/dhanalakshmijonna/"
            className="box font-medium text-white  flex items-start justify-end gap-1"
          >
            <FaLinkedinIn className=" text-white text-xl hover:scale-125 cursor-pointer" />
            <p>Linkedin</p>
          </a>

          <a
            data-aos="fade-up"
            data-aos-duration="1000"
            href="https://github.com/DhanaLakshmi-9998"
            className="box font-medium text-white   flex tems-start justify-center gap-1"
          >
            <AiFillGithub className=" text-white text-xl hover:scale-125 cursor-pointer" />
            <p>Github</p>
          </a>
          
        </div>
        <div className="sm:text-[12px] text-[14px] text-white">
          | Copyright &copy; <span>2024 Dhanalakshmi. </span> All rights reserved
          <a href="#"></a> |
        </div>
      </div>
    </div>
  )
}

export default Footer
